...
Looking at the histogram we can see that it follows a Gaussian, centered around 60-70% with a mean of ~65.46, and a standard deviation of ~17.17. It also has an outlier with a graduation rate more than 100% which obviously cant be the case as its caps at 100%.
...
To achieve my goal, I plan to first use a linear regression model to predict the quantitative responses. I will also experiment with using random forests and decision trees. I also plan to apply k-fold cross-validation and Lasso (in the linear regression case) to ensure the validity of my model, specifically using the Lasso method as I anticipate that not all variables will be linked to the response.
...
For the k-fold cross-validated linear regression with lasso, the R^2 statistic on the test data shows that the model explains the variance ~33.52% of the time which could be better since there is still a large amount of unexplained variance. Also, a high MAE on the test data indicates that the model has a high error level with its predictions. However, the MSE value of ~198.08 is lower than the MSE for linear regression with hold-out cross-validation. This model performs far from desirable with a low R^2, and high MAE and MSE. The diagnostic plot RSME vs. Mixing Percentage shows that increasing the shrinkage parameter effects does not really effect the RMSE which is the measure of average deviation from the actual answer. 
...
To make better informed decisions future studies could collect additional data on student demographics, academic performance, and extracurricular activities. This would help to provide a more comprehensive understanding of the factors that impact graduation rates, and could lead to the development of more accurate predictive models.

<h1 style="font-size: 30px;">Impacts</h1>

The first major impact of this work is that even though we were unable to make an efficient model to predict the graduation rate we are able to say that with this data more complex models are needed to predict the graduation rate, or that the variables within the dataset are not enough to build an accurate model. To improve accuracy it may be needed to include additional predictors with resampling to get a new dataset. Overall, my study indicates that further research needs to be done to identify a model that will accurately predict the graduation rate.

Another major impact would be that it sheds light on what factors correlate with the graduation rate at different colleges. For example, we can say that the graduation rate is influenced by what percentage of the students were in the top echelon of their high school class. Meaning that students that were already on a good track are going to boost their graduation rate regardless of what college they are in. To add on, the number of applications a college gets seems to be correlated with the graduation rate and a reason for that could be that if a college has a large volume of applications they can be picky and choose the best students from their prospective high schools. Lastly, the amount of funding a school gets from alumni donating and how expensive out-of-state tuition is correlated heavily with the graduation rate. Reasons for this could be that better funding could lead to better resources, and higher tuition fees could attract more committed students that were at the top of their high schools.

If people were to believe in our results it would seem that the success of a student in college would not be based on the actual education quality of a college like the quality of teachers, or how much they spend on each student, but was already predetermined by how successful a student was in high school. All of the factors that turned out to be important in predicting graduation rates seem to all influence which top students go to which college. This could lead to colleges unethically raising tuition rates to boost graduation rates or this could influence top students to not care about what college they go to since they are more likely to graduate anyway. If our results are not accurate then this could create distrust between the education system and students in the U.S. unfairly. However, if they are we need to rethink how the college education system is done within the U.S. because the foundations that it’s built upon for students’ success are not the core reasons for students’ success.